...
def render_my_qr_codes_page(request):
    ...
    if "find" in request.POST:
        return render(request, "my_qr_codes.html", context = {"username" : username, "qr_codes": finded_qr_codes, "subscribe": subscribe})
    if "find" not in request.POST:
        return render(request, "my_qr_codes.html", context = {"username" : username, "qr_codes": qr_codes, "subscribe": subscribe})