...
def render_first_step_payment_desktop_page(request):
    error = ""
    ...
    return render(request, "first_step_payment_desktop.html", context = {"error" : error})




def render_second_step_payment_desktop_page(request):
    error = ""
    user_now = request.user
    ...
    if request.method == "POST":
        try:
            ...
            if secret_code_input == secret_code_model:
                return redirect("/third_step_desktop/")
            else:
    ...
    return render(request, "second_step_payment_desktop.html", context = {"error" : error})
